...
class BlockchainController {
    ...
    getBlockByHeight() {
        this.app.get("/block/:height", async (req, res) => {
            console.log ("inside getBlockByHeight");
    ...
    requestOwnership() {
        this.app.post("/requestValidation", async (req, res) => {
            console.log("inside requestOwnership");
            if(req.body.address) {
    ...
    // This endpoint allows requests to validate the chain
    getValidateChain() {
        this.app.get("/validateChain/", async (req, res) => {
            try {
                ...
                return res.status(500).send("Error - could not execute validation");
            }
        });
    }
