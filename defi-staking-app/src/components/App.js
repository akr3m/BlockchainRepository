...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (daiTokenData) {
      ...
      window.alert('DiaToken contract not deployed to detected network');
    }
  ...
  stakeTokens = (amount) => {
    ...
    .send({ from: this.state.account }).on('transactionHash', (hash) => {
      this.state.tokenFarm.methods.stakeToken(amount)
  ...
  unstakeTokens = (amount) => {
    ...
    this.state.tokenFarm.methods.unstakeToken().send({from: this.state.account})
    .on('transactionHash', (hash) => {
      this.setState({loading: false});
    });
  }
