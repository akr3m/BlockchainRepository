...
contract('dBank', ([deployer, user]) => {
  let dbank, token
  const interestPerSecond = 31688088 //(10% APY) for min. deposit (0.01 ETH)
  ...
  describe('testing deposit...', () => {
    ...
    describe('failure', () => {
      ...
      it('depositing should be rejected', async () => {
        await dbank.deposit({value: 10**16, from: user}) //0.01 ETH
        let depositStatus = await dbank.isDeposited(user);
        await dbank.deposit({value: 10**16, from: user}).should.be.rejectedWith(EVM_REVERT) // trying the deposit again for the same address
      })
    })
  })
